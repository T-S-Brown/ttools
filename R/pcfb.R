...
#'
#' Calculates the percent % change in value of a variable over time - typically a baseline value.
...
pcfb <- function(data, var, groupvar = USUBJID,
  ...
  data_out <- inner_join(bl_data, ref_data, by = "USUBJID") %>%
    mutate(pcfb = ((post_base - baseline) / baseline) * 100) %>%
    select(!!key_group, pcfb)

  colnames(data_out)[2] <- paste("pcfb_", rlang::get_expr(key_var), "_",
                                 gsub(" ", "", reftime), sep = "")
